...
const AnalysisBlock = ({ category }) => {
  ...
  const getInterpretationColor = (interp) => {
    if (interp === 'Bullish') return 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30';
    if (interp === 'Bearish') return 'bg-rose-500/20 text-rose-400 border-rose-500/30';
            ...
            <div className="space-y-4 mt-3">
                {category.methods.map((method, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row sm:items-start gap-3 text-sm">
                        <div className="min-w-[140px] font-medium text-slate-300">{method.name}</div>
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-400">{method.details}</p>
                        </div>
                    </div>
                ))}
            </div>
